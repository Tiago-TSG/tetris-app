...
```bash
docker build -t tetris-app-checkpoint-01 .
```
...
```bash
docker run -p 8080:8080 tetris-app-checkpoint-01
```
...
3.  Execute o comando de deploy. Ele criará a imagem e a colocará em execução:
    ```bash
    gcloud run deploy tetris-app-checkpoint-01 \
      --source . \
      --region us-central1 \
...
4.  Ao final do processo, a CLI do gcloud exibirá a **URL pública do jogo** (ex: `https://tetris-app-checkpoint-01-xxxxx-us-central1.run.app`) no serviço "Cloud Run". Basta abrir no navegador e jogar!
...
2.  **Construir a imagem e enviá-la para o GCP via Cloud Build:**
    ...
    ```bash
    gcloud builds submit --tag us-central1-docker.pkg.dev/PROJECT_ID/neon-arcade-repo/tetris-app-checkpoint-01:latest .
    ```

3.  **Realizar o deploy do container armazenado no registro para o Cloud Run:**
    ```bash
    gcloud run deploy retro-neon-tetris \
      --image us-central1-docker.pkg.dev/PROJECT_ID/neon-arcade-repo/tetris-app-checkpoint-01:latest \
      --region us-central1 \
      --allow-unauthenticated
    ```
